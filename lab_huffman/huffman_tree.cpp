                            ...
                            queue<TreeNode*>& mergeQueue)
{
  ...
  if(singleQueue.empty()){
    TreeNode * curr = mergeQueue.front();
    mergeQueue.pop();
    return curr;
  }
  if(mergeQueue.empty()){
    TreeNode * curr = singleQueue.front();
    singleQueue.pop();
    return curr;
  }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while((singleQueue.size() + mergeQueue.size()) > 1){
       TreeNode * curr1 = removeSmallest(singleQueue, mergeQueue);
       TreeNode * curr2 = removeSmallest(singleQueue, mergeQueue);
       TreeNode * newnode = new TreeNode(curr1->freq.getFrequency()+ curr2->freq.getFrequency());
       newnode->left = curr1;
       newnode->right = curr2;
     ...
     // if(singleQueue.empty()){
       ...
       // singleQueue.pop();
     // }else{
     root_ = mergeQueue.front(); //may need to put at the front of singleQueue
     mergeQueue.pop();
     // }
}
     ...
     */
     ...
     if(!current->left && !current->right){
       ...
       bfile.writeByte(current->freq.getCharacter());
        ...
        //potential segfault problem if set == NULL instead of != NULL
