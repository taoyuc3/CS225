...
class KDTree
{
  ...
  private:
     ...
     */
     void buildKDTree(vector<Point<Dim>>& newPoints, int left, int right,
                      int d, KDTreeNode *& subroot);

     Point<Dim> select(vector<Point<Dim>>& elements , int left, int right, size_t k, int d);

     int partition(vector<Point<Dim>>& elements , int left, int right, size_t pivotIndex, int d);

     void copy(KDTreeNode *& copyNode, KDTreeNode *& other);

     void deleteTree(KDTreeNode *& subroot);

     void finder(KDTreeNode* queryNode, Point<Dim> &queryBest, Point<Dim> &query, int d) const;
};
